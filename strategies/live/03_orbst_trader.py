#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  03_orbst_trader.py — Strategy #03: ORB + Supertrend confirm (Version 1)   ║
# ║  Research : scratch/nifty_trend/runs/orb_supertrend/ (p=0.000, Sharpe 2.06)║
# ║  Config   : ../../nifty_config.json  →  key: "orbst_v1"                    ║
# ╚══════════════════════════════════════════════════════════════════════════╝
#
# ┌─────────────────────────────────────────────────────────────────────────┐
# │  LOGIC (NIFTY spot signal → ATM option execution, long + short)          │
# │                                                                          │
# │  Opening range : high/low of first `or_min` mins (default 60: 09:15-10:15)│
# │  Supertrend    : ST(st_period=14, st_mult=3.0) direction filter           │
# │  LONG  (BUY CE): close crosses ABOVE OR_high + orb_k×ATR  AND ST dir = up │
# │  SHORT (BUY PE): close crosses BELOW OR_low  − orb_k×ATR  AND ST dir = dn │
# │  EXIT          : stop_only — spot ATR stop (atr_sl×ATR), NO target,       │
# │                  ride the move to 3:15 EOD (matches backtest exit style). │
# │  Max 2 trades/day. 1x, no leverage. skip-expiry handled by RMS guards.    │
# │                                                                          │
# │  Every order goes through execution_gateway (RMS gate + order_store) —    │
# │  NEW_STRATEGY_CHECKLIST compliant.                                        │
# └─────────────────────────────────────────────────────────────────────────┘
...
DEFAULTS = {
    ...
    "or_min": 60, "orb_k": 1.0, "st_period": 14, "st_mult": 3.0,
    "atr_period": 14, "atr_sl": 1.5,
...
def fetch_nifty(token, cid, tf_min, days=7, use_cache=True):
    """Continuous `tf_min` NIFTY spot bars for the last `days` (ATR+ST warm-up, TRAP #85)."""
    if use_cache:
        try:
...
def _supertrend_dir(df, period, mult):
    """Supertrend direction series (+1 up / -1 down) — SAME recursion as the backtest
    (scratch/nifty_trend/intraday_engine.supertrend), ATR = canonical wilder_atr."""
    a = _atr(df, int(period))
    hl2 = (df["high"] + df["low"]) / 2
    up = (hl2 - float(mult) * a).values
    dn = (hl2 + float(mult) * a).values
    close = df["close"].values
    n = len(df)
    dir_ = np.ones(n)
    fup = up.copy(); fdn = dn.copy()
    for i in range(1, n):
        fup[i] = max(up[i], fup[i - 1]) if close[i - 1] > fup[i - 1] else up[i]
        fdn[i] = min(dn[i], fdn[i - 1]) if close[i - 1] < fdn[i - 1] else dn[i]
        if close[i] > fdn[i - 1]:
            dir_[i] = 1
...
def compute_signal(df, cfg):
    ...
    need = max(period, int(p["st_period"])) + 3
    if len(df) < need:
        return None
...
# ─────────────────────────────── main loop ─────────────────────────────────
def run(paper_mode=True, strategy_id="orbst_v1"):
    ...
    if not acquire_singleton(strategy_id):
        ...
        return
    log.info("=" * 62)
    log.info(f"  03_orbst_trader.py  |  {strategy_id}  |  {'PAPER' if paper_mode else '⚡ LIVE'}")
    log.info("=" * 62)
    ...
    while True:
        try:
            ...
            if not tc.get("active", False):
                log.info("[ORBST] Paused — active=false"); time.sleep(60); continue
            if not is_market_open():
                log.info(f"[ORBST] Market closed ({now.strftime('%H:%M')} IST)"); time.sleep(60); continue

            token, cid = load_creds()
            df = fetch_nifty(token, cid, tf_min)
            if df is None or df.empty:
                log.warning("[ORBST] no candle data"); time.sleep(30); continue
            ...
            if pos is None and trades_today < int(tc.get("max_trades_per_day", 2)):
                ...
                if sig:
                    ...
                    if _fresh is not None and not _fresh.empty:
                        ...
                        if not _sig2 or _d2 != _d1:
                            log.info("[ORBST] signal "+str(_d1)+" NOT confirmed on fresh Dhan candle - skip (TRAP#108 guard)")
                            sig = None
                        else:
                            sig = _sig2
                    else:
                        log.warning("[ORBST] fresh Dhan candle unavailable - proceeding on cached signal (TRAP#108 fail-open)")
                log.info(f"[ORBST] spot={spot:.1f}  pos=flat  trades={trades_today}  "
                ...
                if sig:
                    ...
                    if not res or not res[0]:
                        log.error(f"[ORBST] {sym} {opt_type} contract not found"); time.sleep(30); continue
                    ...
                    if ok:
                        ...
                        log.info(f"  ★ ENTRY {sig['signal'].upper()} → BUY {opt_type} {trad_sym}  "
                                 f"spot={sig['entry_spot']:.1f} stop={sig['stop']:.1f} (ride-to-EOD, ST confirm)")

            _write_watch(strategy_id, sym, spot, pos, tc, now)

        except KeyboardInterrupt:
            log.info("[ORBST] Stopped by user"); break
        except Exception as e:
            log.error(f"[ORBST] Loop error: {e}", exc_info=True)
...
def _do_exit(strategy_id, sym, pos, mode, broker, reason, log):
    try:
        ...
        gw.execute_exit(strategy_id, sym, pos["sec_id"], pos["trad_sym"], pos["qty"],
                        ...
                        tag="ORBST", instrument="options", reason=reason, log=log.info)
    except Exception as e:
...
def _write_watch(sid, sym, spot, pos, tc, now):
    try:
        data = {"updated": now.strftime("%Y-%m-%d %H:%M:%S"), "strategy": sid,
                ...
                "levels": {"or_min": tc.get("or_min"), "st": f"ST({tc.get('st_period')},{tc.get('st_mult')})"},
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Strategy #03 — ORB + Supertrend Trader")
    ap.add_argument("--paper", action="store_true")
    ap.add_argument("--live", action="store_true")
    ap.add_argument("--id", default="orbst_v1")
    args = ap.parse_args()
    if args.live:
