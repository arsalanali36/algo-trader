#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  07_banknifty_trader.py — Strategy #07: Mid-Day ORB on BANKNIFTY (V1)      ║
# ║  Research : scratch/nifty_trend/runs/banknifty_hunt/ (p=0.011, Sharpe 1.46)║
# ║  Config   : ../../nifty_config.json  →  key: "banknifty_v1"                ║
# ╚══════════════════════════════════════════════════════════════════════════╝
#
# ┌─────────────────────────────────────────────────────────────────────────┐
# │  LOGIC — same Mid-Day ORB as #00 but on the BANKNIFTY underlying         │
# │  (new underlying = real portfolio diversification).                     │
# │                                                                          │
# │  Underlying spot : BANKNIFTY index (sec_id 25, IDX_I), 15m bars          │
# │  Opening range   : high/low of the first `or_min` mins (09:15–09:45)     │
# │  Entry window    : 11:00–14:00 (mid-day)                                 │
...
# │  Exit            : SL / target / 3:15 EOD. Max 2/day. 1x.                │
# │  Contract        : nearest BANKNIFTY expiry (MONTHLY — weeklies were     │
# │                    discontinued 2024-11-20) via dhan_master (step 100,   │
# │                    lot from scrip master). skip-expiry: no NEW entry on  │
# │                    the BNF expiry day (0DTE gamma), matches backtest.    │
# │                                                                          │
# │  OR cutoff uses `<= or_end` to match the backtest exactly (TRAP #108).  │
# └─────────────────────────────────────────────────────────────────────────┘
...
DEFAULTS = {
    ...
    "or_min": 30, "orb_k": 1.5, "atr_period": 14, "atr_sl": 2.0, "rr": 1.5,
    "win_start": "11:00", "win_end": "14:00",
    "qty": 1, "max_trades_per_day": 2, "strike_offset": 0, "symbol": "BANKNIFTY",
}
...
def fetch_bnf_15m(token, cid, days=6):
    ...
    try:
        ...
        body = {"securityId": BNF_SEC_ID, "exchangeSegment": BNF_SEG,
                "instrument": BNF_INST, "interval": 15, "fromDate": str(fr), "toDate": str(to)}
        try:
...
def compute_signal(df, cfg):
    """Mid-Day ORB breakout on the last CLOSED bar. OR cutoff `<= or_end` (backtest parity,
    TRAP #108). Returns dict(signal, entry_spot, atr, stop, target) or None."""
    p = cfg
    ...
    or_bars = tday[tday["time"].dt.time <= or_end]        # <= : backtest parity (TRAP #108)
    if or_bars.empty:
        return None
    ...
    k = float(p["orb_k"]); atr_i = float(df["atr"].iloc[i])
    if atr_i <= 0:
        return None
    # backtest (intraday_engine tod_orb) tests BOTH close and prev-close against the
    # CURRENT bar's threshold orh[i]=OR±k·ATR[i] — a fresh cross THIS bar. Match exactly
    # (using the previous bar's ATR for prev-close diverged on ~3/40 bars, TRAP #108 family).
    up_i = or_high + k * atr_i
    lo_i = or_low - k * atr_i
    c_i, c_p = float(df["close"].iloc[i]), float(df["close"].iloc[i - 1])
    side = None
    if c_i > up_i and c_p <= up_i:
        side = "long"
    elif c_i < lo_i and c_p >= lo_i:
        side = "short"
    if not side:
        return None
    ...
    if side == "long":
        stop = c_i - stop_dist; target = c_i + float(p["rr"]) * stop_dist
    else:
        stop = c_i + stop_dist; target = c_i - float(p["rr"]) * stop_dist
    return dict(signal=side, entry_spot=c_i, atr=atr_i, stop=stop, target=target,
...
# ─────────────────────────────── main loop ─────────────────────────────────
def run(paper_mode=True, strategy_id="banknifty_v1"):
    ...
    if not acquire_singleton(strategy_id):
        ...
        return
    log.info("=" * 62)
    log.info(f"  07_banknifty_trader.py  |  {strategy_id}  |  {'PAPER' if paper_mode else '⚡ LIVE'}")
    log.info("=" * 62)
    ...
    while True:
        try:
            ...
            if not tc.get("active", False):
                log.info("[BNF] Paused — active=false"); time.sleep(60); continue
            if not is_market_open():
                log.info(f"[BNF] Market closed ({now.strftime('%H:%M')} IST)"); time.sleep(60); continue

            token, cid = load_creds()
            df = fetch_bnf_15m(token, cid)
            if df is None or df.empty:
                log.warning("[BNF] no candle data"); time.sleep(30); continue
            spot = float(df["close"].iloc[-1])
            cur = df.iloc[-1]

            # ── manage OPEN position: spot stop/target (matches backtest atr_rr) ──
            if pos is not None:
                hit = None
                if pos["signal"] == "long":
                    if float(cur["low"]) <= pos["stop"]:       hit = ("BNF_SL", pos["stop"])
                    elif float(cur["high"]) >= pos["target"]:  hit = ("BNF_TARGET", pos["target"])
                else:
                    if float(cur["high"]) >= pos["stop"]:      hit = ("BNF_SL", pos["stop"])
                    elif float(cur["low"]) <= pos["target"]:   hit = ("BNF_TARGET", pos["target"])
            ...
            if pos is None and trades_today < int(tc.get("max_trades_per_day", 2)):
                ...
                log.info(f"[BNF] spot={spot:.1f}  window={tc['win_start']}-{tc['win_end']}  "
                ...
                if sig:
                    ...
                    if not res or not res[0]:
                        log.error(f"[BNF] {sym} {opt_type} contract not found"); time.sleep(30); continue
                    sec_id, trad_sym, lot_size = res
                    if _is_expiry_day(trad_sym, sec_id):
                        log.info("[BNF] expiry day — entry skipped (0DTE gamma, matches backtest)")
                        time.sleep(30); continue
                    lots = int(tc.get("qty", 1))
                    ok, qty = _do_entry(strategy_id, sym, sec_id, trad_sym, lots, (lot_size or 1),
                                        mode, _order_broker(tc), log)
                    if ok:
            ...
            log.info("[BNF] Stopped by user"); break
        except Exception as e:
            log.error(f"[BNF] Loop error: {e}", exc_info=True)
        time.sleep(30)
...
def _do_entry(strategy_id, sym, sec_id, trad_sym, lots, lot_size, mode, broker, log):
    try:
        ...
        res = gw.execute_signal(strategy_id, sym, "BUY", lots, lot_size, sec_id, trad_sym,
                                seg="NSE_FNO", mode=mode, broker_name=broker, tag="BNF",
...
def _do_exit(strategy_id, sym, pos, mode, broker, reason, log):
    try:
        ...
        gw.execute_exit(strategy_id, sym, pos["sec_id"], pos["trad_sym"], pos["qty"],
                        ...
                        tag="BNF", instrument="options", reason=reason, log=log.info)
    except Exception as e:
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Strategy #07 — Mid-Day ORB on BANKNIFTY")
    ap.add_argument("--paper", action="store_true")
    ap.add_argument("--live", action="store_true")
    ap.add_argument("--id", default="banknifty_v1")
    args = ap.parse_args()
    if args.live:
