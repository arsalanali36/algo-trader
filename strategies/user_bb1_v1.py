...
def backtest(df, cfg):
    ...
    # Prepare plot spec with PINK Bollinger Bands — MUST use the engine's
    # build_plot_spec so the chart can render it (it converts each pandas Series
    # to time-stamped points). The chart draws candles + trade markers itself.
    from _CHARTING import spec as chspec
    plot_spec = chspec.build_plot_spec(df, indicators=[
        {"name": "BB Upper", "series": df["bb_upper"],  "type": "line", "color": "#FF69B4"},  # hot pink
        {"name": "BB Lower", "series": df["bb_lower"],  "type": "line", "color": "#FF69B4"},  # hot pink
        {"name": "BB Mid",   "series": df["bb_middle"], "type": "line", "color": "#FF1493"},  # deep pink
    ])
