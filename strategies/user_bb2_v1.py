...
def backtest(df, cfg):
    ...
    # Plot spec — MUST use the engine helper so the chart can render it.
    # (The chart draws candles + buy/exit markers itself; only add the band LINES.)
    from _CHARTING import spec as chspec
    plot_spec = chspec.build_plot_spec(df, indicators=[
        {"name": "BB Upper", "series": df["bb_upper"],  "type": "line", "color": "#FFFFFF"},  # white
        {"name": "BB Lower", "series": df["bb_lower"],  "type": "line", "color": "#FFFFFF"},  # white
        {"name": "BB Mid",   "series": df["bb_middle"], "type": "line", "color": "#CCCCCC"},  # light gray
    ])
