...
- [~] Task 3b — range_trader migrated to execution_gateway — **DIFF REPORTED 2026-07-13, migration HELD for go-ahead** (live-path file, worklist says flag behavior diff first). Inline equity-route checks (range_trader ~1247-1282: check_drawdown→check_concentration→check_capital + sized_lots + CAPITAL_BLOCKED record + raw place_order) vs `execution_gateway.execute_signal` (gate_entry = same 3 + gating_status + live-liquidity + broker-funds + max-premium-cap; smart_order.execute w/ order-chase). **Behavior deltas flagged:** (1) gateway STRICTER (4 extra gates → may block/size more), (2) order-chasing vs raw place_order (fills differ), (3) inline FAILS-OPEN on risk exception, gateway doesn't, (4) blocked-row recording must stay in caller. Awaiting user confirm before changing.
...
- [ ] Task 4 — webhook_executor gateway migration (paper-verified)
- [ ] Task 5 — loop automation set up (audit-loop, eod-review, hunt-pipeline)
