...
**Commit map (2026-07-13 session):**
`b68cdbe` Task 6 + Task 1 · `85de33d` Task 3a + Task 3c · `e32bf3e` Task 3b · `1384058` Task 4 · `d5b6a17` Task 5.
Task 0 = verification/report only (no code). All seven session commits (`b68cdbe`·`85de33d`·`e32bf3e`·`1384058`·`d5b6a17`·`cbbe00e`·`4e0c6b0`) confirmed **already on `origin/master`** (HEAD `712b56c`).

- [x] **✅ POST-MARKET VPS DEPLOY — DONE 2026-07-13 ~15:52 IST (scheduled autonomous task).** Method: per-file md5 diff (local git-HEAD content vs VPS) + tar deploy (path-safe) — not `git pull` (task-specified, and avoids leaking the concurrent in-progress `9fad614` calendar commit). **Safety:** confirmed **ZERO live-mode open positions** before touching anything (order_store scan: 3 open but all `mode:paper` from disabled `ARS_CHAIN_V1_PAPER` = stale netting artifacts, ₹0 real risk; `KillMode=process` = restart doesn't touch strategy PIDs either). **Scope:** of 350 tracked files → 114 identical, 111 CRLF-only (skipped, logically identical), 3 config excluded. **106 files deployed** (52 real content-drift + 70 absent-on-VPS, minus exclusions). Covered the drifted-6 (`02/03/05/07` traders + `execution_gateway`; `04_chainzone`/`06_shortvol`/`risk_gate` were already content-identical on VPS) + this session's `webhook_executor`·`range_trader`·`universe_trader`·`trader_dashboard`·`templates/index.html`·scratch tooling (`data_integrity`/`run_hunt`/`optlake_load`/`real_struct2`/`SLIP_RECOST`)·`scripts/setup_hooks.sh`·ADRs·`.claude/commands/*`·docs. **Excluded** (preserved VPS-own): all `data/`, `nifty_config.json`, `data/config.json`, `range_config.json` (runtime `active:true` state), whole `_PINE/**` (has `sync_pine.py` union-merge), `bs_option.py` (uncommitted local scratch — HEAD already on VPS), 5 unicode/space reference CSVs (deferred, non-critical validation data). **md5-verified: all 106 local==VPS.** Pre-deploy backup: `/root/code3b_predeploy_backup_20260713_154628.tgz`. `setup_hooks.sh` ran → pre-commit hook installed on VPS clone. All 34 changed `.py` py_compile clean; `_core/execution_gateway`+`webhook_executor`+`risk_gate`+`order_store` import-smoke clean. Restarted `algo-dashboard`+`algo-monitor` — both `active`, `NRestarts=0`, logs clean (dashboard serving 200s + Options Cache built; monitor restored trailing-lock peak + ltp_poller started; no import errors/tracebacks). `/lab` route + `runs/` dir verified (double-click strategy → `/lab/runs/<slug>/index.html` works). **PENDING (not done, needs user):** (i) **GitHub push** — only unpushed commit = `9fad614` (concurrent calendar work, in-progress, NOT this deploy) → left for user's explicit OK; (ii) **webhook live paper-verify** — `webhook_v1` is PAPER + confirmed gateway-wired (`webhook_executor:658 gw.execute_signal`) + offline-parity already PASS; a post-market signal-replay is uninformative (no-entry gate closed after 15:15) → observe `webhook_v1` in paper during market hours before any LIVE flip. Signal-replay run flagged orb/orbst strategy drift but that's confounded (today's live entries predate the just-deployed code — re-run tomorrow for a clean read). VPS git HEAD still `4d5fb73` (files updated via file-deploy, not `git pull`) — reconcile VPS git state at leisure.
...
- [x] **Task 12 — Unified Strategy ID system (scalable/stable/logical, user-requested)**. Phase 1 + Phase 2 DONE 2026-07-13 (VPS-deployed). Phase 2 wired ALL identity surfaces to the registry `NN.MM - Name`: logs sidebar, RMS per-strategy table (index.html MISSION_* now registry-driven), Orders/Positions + RMS-summary + webhook rows (regLabel()), Reports (eod_report.py _sid_id/_sid_lbl), Hub (hub.html fetches /api/strategy-registry). Internal keys unchanged. **Phase 3 DONE 2026-07-13:** unified Strategy Registry view at `/registry` (nav '🗂️ Strategies') — every strategy (deployed/research/legacy/backtest) in ONE place as a family tree with ID·Name·status·config-key·file·Lab-link, search + status filters, live running-state from /api/config (`templates/strategy_registry.html` + route). Directly fulfills the 'sab ek jagah rahe' ask. **Phase 4 (optional/maybe-never):** rename internal keys to IDs. Commits 4d48343/6ba42bb/6e7218d/9e013be. --- Phase 1 detail: `strategy_registry.json` (Scheme B family.member — 00=ORB, 01=Chain-Zone, 02=Vol… ; append-only, IDs permanent, rejected keep ID) + `_core/strategy_registry.py` (resolve/label/tree/next_member/next_family/add). Display-only layer (internal config_keys/correlationId UNCHANGED — user-confirmed safe). **Phase 2 PARTIAL DONE 2026-07-13:** `/api/strategy-registry` endpoint + `loadStrategyRegistry()` in index.html → **logs sidebar + RMS per-strategy table** now show registry `family.member - Name` grouped by family (hardcoded seed = graceful fallback). VPS-deployed (surgical route insert, no trader_dashboard.py full-overwrite = drift-safe; PIDs intact). **STILL PENDING in Phase 2:** Orders/Positions rows, Reports (eod_report.py — Python, use `strategy_registry.label()`), hub.html — wire these to show `ID - Name` too. **Phase 3:** unified "Strategy Registry" tree view in app + fold scattered strategies/backtest + hub into it. **Phase 4 (optional/risky, maybe never):** rename internal keys. Proposal doc: `STRATEGY_ID_PROPOSAL.md`.
- [ ] **Task 11 — 🔴 RMS RE-RUN (user-requested, remind until done)**: the 8 mission strategies now have per-strategy RMS caps (nifty_config, orb-ratio: straddle 4500/13000, dvert 17000/5500, orbst 10000/9500, chainzone 12000/11000, backspread 9500/9000, banknifty 12000/17000, orb_v1 10000/15000). Hub's current Sharpe/net were computed under the pipeline's dummy ₹20k/₹30k spot-notional RMS pass, NOT these real premium caps → they WILL alter the result (profit-targets sit below best-day, cut winners). Re-run/re-cost each strategy WITH its own per-strategy caps (LESSONS #104 two-stage), produce before/after (hub vs capped Sharpe), don't overwrite hub without showing delta. Memory [[project_code3b_ACTIVE_rms_rerun_pending]] + weekday scheduled reminder `code3b-rms-rerun-reminder`. Remind user every CODE3B session until done.
