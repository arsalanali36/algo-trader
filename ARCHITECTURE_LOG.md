...
## 2026-07-24 — FII-Flow big-player + option-chain confluence (data-lake + daily collector + verdict: direction FAILS)
**Status:** DONE (data lake + daily Task permanent; direction-prediction concluded NEGATIVE)
**Kya:** Sensibull-style FII/DII participant data ko automate karke, big-player positioning + option-chain milaake next-day NIFTY directional bias (%) banana tha. Data = 100% free NSE. Do engine → confluence → sirf dono-agree pe trade (pro-desk style).
**Layer:** data (new lake), research (backtest)
**Files:** `_ops/fii_flow.py` (participant OI + FII/DII cash; `--daily`/`--backfill`/`--rebuild-master`), `_ops/chain_pcr.py` (NIFTY daily PCR near/all + max-pain from NSE FO bhavcopy — old + UDiFF auto-detect, parallel `--workers`), `_ops/fii_flow_daily.bat` + Windows Task `KHAZANA_FII_Flow_Daily` (weekdays 19:30 IST), `scratch/fii_flow/{fii_backtest,confluence_backtest}.py` + `README.md`. Lake: `._TRADING DATA/FII_Flow/` (raw_oi/, cash/, fii_flow.csv 2015→, chain_pcr.csv 2020→).
**Verify:** 23-Jul FII cash −2999.23/DII +2947.14 == Sensibull screenshot EXACT (data validated). Old-fmt bhavcopy parses 2016→, UDiFF 2024→ (PCR/max-pain sane on both). Parallel backfill ~17x serial (12→210 din/min).
