...
**Teeno flagged gaps usi session me FIX ho gaye (user ne bola "teeno hi fix kar dijye") — commit `e02cc97` + hotfix:**
- **(1) `_today_open` today-scoped → positional legs RMS ko dikhte hi nahi the** (live condor ~₹85k margin = ₹0 gina ja raha tha; TRAP #119 ka mirror — jis today-scoping ne dashboard se positional chhupaayi, usi ne risk engine se bhi chhupaayi). Fix: prior-day open legs carry-over, **sirf opt-in `allow_overnight`** strategies ke — intraday strategy EOD pe flat hoti hai, uski prior-day "open" rows STALE hain (is DB me ~38), unhe ginna phantom capital bana ke sab kuch false-block kar deta. Verified: `capital_in_use(paper)` ₹80,247 → ₹1,53,410, stale 38 nahi gine.
- **(2) 9 mission families ke 20+ exit reasons** (`ORB/ORBST/BNF/CHAIN/STRADDLE/DVERT/BSPRD/SVOL/VRP/VRPC`) `_EXIT_REASON_PREFIXES` me the hi nahi → `_exit_reason()` unrecognised maan ke drop karta → **har mission exit ka reason blank**. UI badges aadhe pehle se maujood the = feature ek-tarfa land hua tha. Ab **FAMILY prefixes** (startswith) → naya `<FAM>_TRAIL` variant likhte hi cover. Teesri baar yehi gap (TRAP #88, #91).
- **(3) Har mission trader `trades_today = 0 if pos is None else 1` se seed karta tha** → position band hone ke BAAD koi bhi restart counter 0 kar deta aur poora max-trades quota wapas mil jaata (live: orbst 1→0). Naya `risk_gate.entries_today()` durable order_store se padhta hai; 8 traders switched (read fail ho to purana guess fallback). `daily_max_trades_hit()` bhi wahi share karta hai — jisse uska **asli undercount** bhi fix hua: wo sirf `details` (completed) bucket padhta tha, to still-OPEN entry RMS cap me ginti hi nahi thi (docstring "open + completed" claim karta tha, code kabhi nahi).
- **Meri apni galti (TRAP #121):** patch `risk_gate.entries_today()` ko module-function scope se call karta tha par zyadatar traders `risk_gate` sirf DOOSRE functions ke andar import karte hain → 7 traders startup pe `NameError` se mare. Maine `grep -c "import risk_gate"` se "verify" kiya tha — usne function-local import gin liya = **jhoota PASS**. Hotfix: call site pe local import + verification ab 8/8 modules ka **asli import** (grep nahi). Positions safe rahe (sab paper + pos_monitor order_store se independent protect karta hai).
...
## 2026-07-18 — Markov / Jim-Simons video study → NIFTY momentum → ₹1L after-tax reality
**Status:** DONE (research, `scratch/markov_meanrev/`; display-only, NO live strategy; stock cross-sectional + pledge-stacking pending overnight data — scheduled task fires 2026-07-19 14:00 IST)
**Kya:** Reproduced the Medallion/Jim-Simons "Markov chain" video's buy-the-dip (N consecutive down closes → long, exit on up close). Verdict across 5 tests: (1) NIFTY daily = momentum, edge INVERTS; (2) NIFTY/BNF 5-min = real reversion edge (p=0.000, gross Sharpe~1.7) but per-trade 0.3-0.9bps << ~2bps cost → NET collapses (HFT-class, below retail cost floor); (3) 20-stock + (4) 215-stock daily basket = no edge, loses to B&H even gross ("buy unusually-low" = falling-knife, −57%); (5) high-VIX regime = video's "reversion in high-vol" directionally right but marginal (PF~1.1). **Flip to MOMENTUM** (Markov predicted: up→up 0.562): NIFTY daily breakout (L50/H10) but **PROPER validation (permutation significance vs random-entry null) KILLS it — whole family p=0.16-0.35, NOT significant.** Sharpe/DD edge over B&H is pure LOW-EXPOSURE (out of market ~64%, 50d-high can't fire in a crash) = defensive exposure, not timing alpha. **₹1L real-money model (NIFTYBEES ETF, real Indian charges + STCG/LTCG tax):** NIFTY buy&hold ₹2,29,179 / 10.2% CAGR BEATS strategy (best ₹1,78,594 / 7.0% only if idle cash earns 6% liquid; else ₹1,28,521 / 3.0%). Charges tiny (₹3,114/71 trades); drag = STCG 20%×71 vs B&H's single 12.5% LTCG. **User's conclusion (correct): simple NIFTY buy&hold wins — "itna dimag kyun lagayen".**
**Layer:** research (scratch/markov_meanrev)
**Files:** `_common.py` (1min→daily + any-CSV loader), `markov_analyzer.py` (transition matrix + streak significance), `markov_scan.py` (multi-TF/instrument), `markov_intraday_bt.py`, `stock_meanrev.py`, `stock_universe_bt.py` (--grid/--momentum/--focus), `markov_regime.py` (VIX split), `index_momentum_bt.py` (NIFTY/BNF 8.5yr), `nifty_momentum.py` (validated build: sweep+train/OOS+significance+MonteCarlo→JSON), `nifty_1lakh_model.py` (real charges+tax). Pine: `_PINE/markov_meanrev_v1.pine` (video repro) + `_PINE/momentum_breakout_v1.pine` (committed f227b61). Memory: `project_code3b_markov_meanrev_nifty`.
**DATA UNLOCK:** Dhan `/v2/charts/intraday` gives 1-min STOCK data back to ~2015 (full from 2019) — lake was only 1.5yr because `download_equity_history.py` defaulted `--from 2025-01-01`. Deep backfill of all 210 F&O stocks 2015→today launched on VPS (`nohup ... --universe fno --from 2015-01-01`, log `/root/equity_deep_backfill.log`); `algo-equity-daily.timer` keeps it current. Enables NIFTY-style long/robust STOCK tests.
**Verify:** significance permutation (1000 nulls) reproduced across L50/L100/L200; ₹1L model itemised (STT/txn/stamp/SEBI/GST/DP + STCG/LTCG), transparent assumptions (NIFTYBEES STT 0.001% sell-only flagged). Data not in git (regenerable from lake / `_common.py`).
**DO-NOT-REDO:** NIFTY single-index price-only timing (mean-rev AND momentum both fail significance — buy&hold wins after tax). Intraday micro-reversion (below cost). **Next (scheduled 07-19):** stock cross-sectional momentum (different mechanism, documented factor) + pledge-stacking test (uncorrelated-only adds return, else = leverage).
