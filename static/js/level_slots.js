...
(function () {
  ...
  function renderProjection(d) {
    ...
    const C = d.cur || '₹';
    const legs = (d.legs || []).map(l => `<div><span style="color:${l.side === 'SELL' ? 'var(--red)' : 'var(--green)'}">${l.side}</span> ${l.sym} · strike ${fmt(l.strike, 0)} · LTP ${C}${fmt(l.ltp, 2)}${(l.bid || l.ask) ? ` · <span style="color:var(--muted)">bid</span> ${fmt(l.bid, 2)} / <span style="color:var(--muted)">ask</span> ${fmt(l.ask, 2)} → fill <b>${fmt(l.fill, 2)}</b>` : ' · <span style="color:var(--muted)">bid/ask: feed idle</span>'} · Δ ${l.delta == null ? '—' : l.delta}</div>`).join('');
    ...
    el.innerHTML = `${legs}
      <div style="margin-top:4px">net |Δ| <b>${d.net_delta == null ? '—' : d.net_delta}</b>/unit · <b>${d.rs_per_pt == null ? '—' : '₹' + fmt(d.rs_per_pt, 1)}</b> per index pt · qty ${d.qty}${d.lot_size ? ' (lot ' + d.lot_size + ')' : ''} · credit (LTP) ${C}${fmt(d.credit_unit, 2)}/unit = <b>${money(d.credit_rs)}</b>${d.credit_fill_unit != null ? ` · <b>at bid/ask ${C}${fmt(d.credit_fill_unit, 2)} = ${money(d.credit_fill_rs)}</b> <span style="color:var(--muted)">(spread cost ${money(-Math.abs(d.spread_cost_rs || 0))})</span>` : ''}${d.atm_iv ? ' · IV ' + (d.atm_iv * 100).toFixed(1) + '%' : ''}</div>
