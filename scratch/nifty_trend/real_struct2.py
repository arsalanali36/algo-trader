    ...
    if -10 <= off <= 10:
        ...
        if STRICT:
            raise LakeCoverageError(
                ...
                % (K, off, g["ATMK"][i], i))
    S = g["SPOT"][i]
    return max(0.0, (S - K) if side == "CE" else (K - S))  # intrinsic-ok: kept for the ~50 legacy call sites, but no longer silent — counted in _OOB/oob_report() and raises under STRICT; the publish path asserts coverage (_assert_lake_coverage, TRAP #198)   # intrinsic floor fallback
