...
def _orb_overlay(d, atr, dts, p, side, entry_dt):
    """Opening-range box + the breakout trigger that fired + SL/target bracket for one
    ORB-family trade. Levels use the SAME formulas as intraday_engine (OR±k·ATR, entry
    ∓ atr_sl·ATR at the signal bar) so the chart matches the backtest by construction."""
    ...
    return {
        ...
        "lines": [
            {"v": round(fired, 1), "label": ("▲ Break trigger" if side == "long" else "▼ Break trigger"),
...
def build_overlays(design, params, tf, trades, csv="nifty_1min.csv"):
    ...
    for t in trades:
        ...
        if key in overlays:
            continue
        ov = _orb_overlay(d, atr, dts, params, t["side"], key)
        if ov:
            overlays[key] = ov
