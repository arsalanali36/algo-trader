"""build_run.py — emit a schema-compliant runs/m_pattern_ironfly/ from the REAL-lake
IV-pop -> M-rollover iron-fly backtest (bt.deployed_rows()), so the Strategy Lab hub +
registry (02.18) show Sharpe/MaxDD/Win/Trades/Signif/Created + a Lab detail page.

Deployed config = medium-M double-top + hold ~1 trading day + ATM iron-fly +-250 + 50%
credit. Numbers REAL premium (not BS), 5 lots. Entry-timing permutation p=0.009 (real),
Sharpe 0.64 (<1) -> FORWARD-PAPER candidate, not real money (Rule 10).
RESULTS_SCHEMA.md compliant (metric helpers mirror weekly_ironfly/build_run.py)."""
import os, json, io
import numpy as np
...
SLUG = "m_pattern_ironfly"
START_CAP = 1_000_000
LOT = 65
LOTS = 5
QTY = LOT * LOTS

import bt  # sibling module
rows = sorted(bt.deployed_rows(), key=lambda r: r["date"])
for r in rows:
    r.setdefault("rolls", 0)
...
def _all_trades(rr):
    out = []
    for r in rr:
        credit = r["entry_credit"]; net_u = r["net"] / QTY
        out.append(dict(
            side="short", opt_type="fly CE+PE", strike=r.get("atm") or round(r["spot0"] / 50) * 50,
            entry_dt=r["date"] + " " + f'{r.get("entry_hm",920):04d}'[:2] + ":" + f'{r.get("entry_hm",920):04d}'[2:],
            exit_dt=r.get("exit_date", r["date"]) + " 15:20",
            entry_spot=r["spot0"], exit_spot=r["spot0"], points=0,
            entry_prem=round(credit, 2), exit_prem=round(credit - net_u, 2), qty=QTY,
            gross=round(r["gross"]), fee=round(r["charges"]), pnl=round(r["net"]),
            bars=0, reason=f"{r['reason']}"))
    return out
...
def _monthly(rr):
    ...
    for r in rr:
        ...
        m.setdefault(y, {}).setdefault(mo, 0.0); m[y][mo] += r["net"] / START_CAP * 100
...
def _significance(rr, iters=1000, seed=11):
    ...
    net = np.array([r["net"] for r in rr], float)
    real_sh = _metrics(rr)["sharpe"]; dem = net - net.mean()
    yrs = _metrics(rr)["years"]; nulls = []
...
PERM_P = 0.009   # entry-time permutation (random-entry null, 1000 iters) — the decisive test
candles = [[r["date"], r["spot0"], r["spot0"], r["spot0"], r["spot0"]] for r in rows]


def combo(rr):
    m = _metrics(rr); eq, bench, uw, labels, wp = _curves(rr)
    return dict(dna=dict(dist=0, wing=250, take=0.5, structure="iron_fly", lots=LOTS),
...
combos = {"bs|full": combo(rows), "bs|train": combo(tr), "bs|oos": combo(oos),
          "full": combo(rows), "train": combo(tr), "oos": combo(oos)}

meta = dict(window=[rows[0]["date"], rows[-1]["date"]], days=len({r["date"] for r in rows}),
            start_cap=START_CAP,
            design="IV-pop -> M-rollover Iron-Fly — ATM straddle + +-250 wings, entered on an ATM combined-premium double-top rollover, held ~1 trading day, 50% credit target (REAL-lake, 5 lots)",
            tf="intraday-signal / ~1-day hold", candles=candles, passes=["bs"], periods=["full", "train", "oos"],
...
index_entry = dict(slug=SLUG, design="m_pattern_ironfly",
                   title="02.18 - IV-pop M-rollover Iron-Fly (REAL-lake, 5 lots)",
                   tf="intraday-signal / ~1-day hold",
                   params=dict(dist=0, wing=250, take_pct=0.5, structure="iron_fly", lots=LOTS, m_strictness="medium", hold_days=1),
                   exit="50% credit / +1 trading day", instrument="NIFTY 50", lot_size=LOT,
                   window=meta["window"], days=meta["days"], significant=(PERM_P < 0.05),
                   ...
                   p_value=PERM_P, created="2026-08-28 12:00", deploy_key=SLUG, deployed=SLUG,
                   real_cost=dict(method="REAL OptChainLake_1m premium + date-aware Zerodha charges, 5 lots",
                                  note=("Entry-time permutation p=0.009 (real +Rs 3.6L vs random-entry null mean -Rs 3k, z=2.34) "
                                        "-> M-timing edge REAL. Bootstrap p=%.3f. BUT Sharpe %.2f < 1 and config picked from a 9-cell "
                                        "search -> FORWARD-PAPER candidate, NOT real money (Rule 10). Hardening (basket SL / stricter "
                                        "spike) both HURT -> base config is best. No SL: wings = defined-risk cap." % (p_full, mf["sharpe"]))))

with io.open(os.path.join(RUNS, SLUG, "meta.json"), "w", encoding="utf-8") as f:
    json.dump(dict(index_entry), f, indent=1, default=str)
...
print(f"  FULL  sharpe {mf['sharpe']}  net {mf['net_pct']}%  net_abs Rs {mf['net_abs']:,}  maxdd {mf['maxdd']}%  "
      f"win {mf['win_rate']}%  PF {mf['profit_factor']}  trades {mf['trades']}  perm_p={PERM_P}  boot_p={p_full}")
