"""build_run.py — emit a schema-compliant runs/weekly_ironfly/ from the REAL-lake weekly
iron-fly backtest (results.json), so the Strategy Lab hub + registry (02.17) show
Sharpe/MaxDD/Win/Trades/Signif/Created + a Lab detail page.

Numbers are REAL premium (not BS), 5 lots — the deployed spec: ATM straddle + ±250 wings,
enter day-after-expiry 09:20, exit at 50% of net credit else weekly expiry.
RESULTS_SCHEMA.md compliant (metric helpers mirror strangle_roll/build_run.py)."""
import os, json, io
import numpy as np
...
SLUG = "weekly_ironfly"
VARIANT = "wing250 | TARGET 50% of credit"      # the deployed spec
START_CAP = 1_000_000
LOT = 65
LOTS = 5
QTY = LOT * LOTS

data = json.load(io.open(os.path.join(HERE, "results.json"), encoding="utf-8"))
rows = sorted(data["variants"][VARIANT]["rows"], key=lambda r: r["date"])
for r in rows:
...
meta = dict(window=[rows[0]["date"], rows[-1]["date"]], days=len({r["date"] for r in rows}),
            start_cap=START_CAP,
            design="Weekly Iron-Fly — ATM straddle + ±250 wings, day-after-expiry 09:20, 50% credit (REAL-lake, 5 lots)",
...
index_entry = dict(slug=SLUG, design="weekly_ironfly",
                   title="02.17 - Weekly Iron-Fly (day-after-expiry, REAL-lake, 5 lots)", tf="positional",
                   params=dict(dist=0, wing=250, take_pct=0.5, structure="iron_fly", lots=LOTS),
                   ...
                   p_value=p_full, created="2026-08-24 12:00", deploy_key=SLUG, deployed=SLUG,
                   real_cost=dict(method="REAL OptChainLake premium + date-aware Zerodha charges, 5 lots",
                                  note="real premium (not BS) — seller, trustworthy per TRAP #136. No SL: wings = defined-risk cap."))
...
print(f"  FULL  sharpe {mf['sharpe']}  net {mf['net_pct']}%  net_abs Rs {mf['net_abs']:,}  maxdd {mf['maxdd']}%  "
      f"win {mf['win_rate']}%  PF {mf['profit_factor']}  trades {mf['trades']}  p={p_full}  signif={p_full<0.05}")
