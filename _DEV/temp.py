...
def run_backtest(date_to_test="2026-05-06", timeframe="5min"):
    ...
    for i in range(2, n):
        ...
        t = row["datetime"]
        atr_val = float(atr_series.iloc[i]) if not pd.isna(atr_series.iloc[i]) else 5.0
        ...
        if t.time() == pd.to_datetime('13:05').time():
            print(f'DEBUG 13:05: touch={touch_active}, bull={bullish}, bear={bearish}, active_type={active_touch_type}, cs={h-l}, max_cs={max_cs}')
