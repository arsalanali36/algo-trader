...
**PENDING (Parts 2/3 — bhai wants full root, not patch):** (2) seal `externally_closed`/cross-group leg cross-pollution inside global `_net_rows` (a leg from a resolved/different group must not FIFO-pair with an unrelated future leg) — needs a full before/after whole-DB comparison test since ~30 consumers depend on `_net_rows`. (3) clean the **110** existing `externally_closed` leftover legs (backup → mark inert) + stop accumulation. Memory `project_code3b_group_leg_resolution`, ADR-019.
