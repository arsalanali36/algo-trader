...
## TRAP #62 — Account-level trailing-profit-lock squareoff closes a position at the broker but never tells the owning strategy process — the strategy can later try to "exit" a position that's already gone, placing a real, unintended order 🟡 (flagged, not yet fixed — user's call)

**Symptom:** ~40 minutes after `pos_monitor_loop`'s account-level trailing-profit-lock squareoff closed a SUNPHARMA SELL position (a mechanism separate from the strategy's own exit logic — see Critical Rule in `CLAUDE.md` re: trailing_profit_lock_rs/pct), the strategy process (`range_trader.py` / `ARS_CHAIN_V1`) logged `EXIT SUNPHARMA via ATR_TRAILING` and placed a real BUY order — treating it as "closing my short position." But there was no short position left to close (it had already been bought back by the trailing-lock 40 minutes earlier) — the strategy's own in-memory `_state[symbol]["position"]` was never told about that closure, so it kept believing a position was open until its own ATR-trailing-stop condition eventually fired. The resulting BUY was a real, unintended fresh long entry disguised as an exit in the strategy's own (stale) worldview — user caught it and manually flattened it (~₹122.50 cost, on top of triggering TRAP #61's display bug).

**Root cause:** Two independent systems can both close the same position — (1) the strategy's own per-symbol exit logic (zone/ATR/target, `range_trader.py`'s own `_state`), and (2) the account-level trailing-profit-lock in `pos_monitor_loop` (`trader_dashboard.py`), which squares off EVERYTHING once account P&L drops enough from its peak. `_recover_state_from_order_store()` (TRAP #28) already handles this gap for the RESTART case, rebuilding `_state` from `order_store` on startup — but there's no equivalent LIVE sync: if the trailing-lock closes a position while the strategy process keeps running (no restart), the strategy's in-memory state is never told and stays stale until it independently tries to act on it.

**Not fixed this session** — flagged for a future session. Two possible directions: (a) trailing-lock squareoff could write a signal file / call a strategy-process endpoint to clear the relevant `_state[symbol]`, or (b) `range_trader.py`'s own loop could periodically re-validate its `_state` against `order_store`'s current open positions (similar to what `_recover_state_from_order_store` already does at startup, just repeated live) and self-correct if it finds a position it thinks is open that order_store shows as already closed by someone else.
...
**Not fixed in this pass (flagged separately):**
- `ICICIBANK-Jul2026-1330-PE`'s never-filled provisional row (id 386, TRAP #58) is still sitting as `open` in `order_store` — confirmed via `kite.order_history()` it was CANCELLED (never traded, filled_quantity 0 throughout), so it's a pure phantom, not a live risk. It hasn't self-healed via `broker_sync`'s regular ghost-sync because `_check_flat()`'s Kite branch can't distinguish "genuinely zero history for this contract" from "uncertain" — Kite's `positions()` simply omits a contract entirely if it never had ANY fill, and the current code treats "not present in the response" as "uncertain → assume still open" (same safe-default convention used everywhere else in `_check_flat`, just wrong for this specific edge case). Needs checking order status directly (not just positions) as a cleanup path — not done yet.
...
**Fix:** `range_trader.py`'s `main()` now calls `dhan_feed.start({"client_id":..., "jwt_token":...}, [])` once at startup (same pattern `trader_dashboard.py`'s `_ensure_feed_started()` already uses) — actually launches the WebSocket thread this time, so `add()` calls later in the run have a live connection to attach to.

**Not fully closed — flagged, not fixed this pass:** even with the feed running, a symbol's *very first* signal of the day is still a cold subscribe — `dhan_feed.add()` is called reactively (right before a premium fetch), not proactively for the day's whole symbol universe at market open, so there may not be a tick yet by the time that first liquidity check runs. Fully closing this would mean subscribing all 25 universe symbols' likely ATM contracts (or at least the underlying) at daily-levels-load time, giving the feed a head start before any signal can fire. Also unverified: whether the REST fallback's real-world failure rate under load is itself worth hardening (retry/backoff) now that it's the cold-start-only path instead of the every-single-call path it was carrying until now.
