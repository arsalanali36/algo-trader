...
**161 modules documented** across 10 folders.
...
- 🔧 `green_hammer` — …
- 🔧 `inside_bar` — Inside bar: current candle's whole range sits inside the previous one
- 🔧 `inv_red_hammer` — …
- 🔧 `is_bearish_pattern` — …
- 🔧 `is_bullish_pattern` — …
- 🔧 `red_hammer` — …
...
- 🔧 `assess` — …
- 🔧 `load_backtest` — Run ka per-UNIT net array + uska apna MC worst-5% DD (per unit). None if absent.
- 🔧 `load_live` — Live/paper completed trades -> per-UNIT NET array (charges ke baad), time-ordered.
- 🔧 `main` — …
- 🔧 `print_report` — …
- 🔧 `run` — …
...
- 🔧 `candles` — Recent OHLC candles for the perpetual (spot proxy) — `/v2/history/candles`.
...
- 🔧 `registry_vs_config` — Registry ka declared `status` vs live config ka `mode`/`active` — mismatch list.
- 🔧 `run` — …
- 🔧 `safe_mode_status` — …
...
- 🔧 `active_slots` — Slots the watch loop must evaluate (day-rolled).
- 🔧 `add_underlying` — …
- 🔧 `advance` — Pure state step. `bars` = CLOSED candles oldest→newest (dicts with
- 🔧 `apply_runtime` — Persist the runtime fields produced by advance() (config untouched).
- 🔧 `arm` — Arm a configured slot. Refuses if: no level, already fired today, past
- 🔧 `break_confirmed` — Did the candle AFTER the pattern candle break it?
- 🔧 `candle_in_zone` — …
- 🔧 `claim` — Atomic one-shot: pattern-confirmed slot → firing (persist BEFORE any order).
- 🔧 `delete_watchlist` — …
- 🔧 `detect_pattern` — Name of the first matching pattern on `cur` (closed) given `prev`, or None.
- 🔧 `disarm` — …
- 🔧 `ensure_fixed` — Seed the 3 fixed tabs (idempotent) — called at page/loop start.
- 🔧 `get_slot` — …
- 🔧 `is_bearish` — from below (resistance) → bearish → sell CE, break = pattern LOW.
- 🔧 `is_expired` — valid_till crossed? Date-less value = today's wall-clock; dated value = full IST datetime.
- 🔧 `level_at` — Effective level: trend line value at bar time `t` (extrapolated), else the fixed level.
- 🔧 `list_slots` — …
- 🔧 `list_underlyings` — …
- 🔧 `list_watchlists` — …
- 🔧 `normalize_valid_till` — 'HH:MM' (aaj) ya 'YYYY-MM-DD HH:MM' → canonical string, invalid → None.
- 🔧 `now_bar_time` — Current IST wall-clock as the IST-shifted epoch the bars use (for level_at(now)).
- 🔧 `option_side` — (opt_type_to_SELL, dir_for_exit_rule): bearish → CE, dir -1; bullish → PE, +1.
- 🔧 `remove_underlying` — …
- 🔧 `save_slot` — Upsert config on an existing slot. Armed/active slot ka config badalna =
- 🔧 `save_watchlist` — Upsert a named list. symbols = list of underlying symbols (upper-cased, de-duped,
- 🔧 `set_result` — …
- 🔧 `set_status` — …
- 🔧 `validate_config` — Pure validation of a slot's user config → (ok, cleaned_or_err).
- 🔧 `zone_band` — …

### `_ops/level_slots_live.py`
level_slots_live.py — watch loop + entry firing for _ops/level_slots.py.

- 🔧 `candle_source` — ('delta', None, None, None) | ('dhan', sec_id, seg, instrument) | None
- 🔧 `contracts_near_atm` — Strike picker for PREMIUM slots: ATM±n contracts of the nearest expiry.
- 🔧 `fetch_bars` — Closed candles for the slot's source at its TF (oldest→newest, IST-shifted
- 🔧 `fire_slot` — …
- 🔧 `fno_symbols` — …
...
- 🔧 `preview_structure` — What the slot WOULD do right now (no order): legs (strike/LTP/Δ), net |Δ|/unit,
...
- 🔧 `underlying_info` — (sec_id, seg, instrument) for candles/spot of an NSE underlying, or None.
- 🔧 `watch_loop` — …
- 🔧 `watch_once` — One evaluation pass over every armed slot (+ BTC exit checks). Returns #fired.
...
- 🔧 `advance_confirm` — Pure confirmation state machine → (new_state, fire).
- 🔧 `check_exit` — Pure exit decision on a group's live combined MTM (₹, whole position).
- 🔧 `clear_rule` — …
- 🔧 `get_rule` — …
- 🔧 `is_beyond` — Pure → is `price` past `level` on the exit side of it?
- 🔧 `list_rules` — …
- 🔧 `rule_key` — Canonical identity for a group — prefer the durable group_id link, else
- 🔧 `set_rule` — Store a group's exit rule.
- 🔧 `trigger_levels` — Pure → every ENABLED index-based trigger as {src, side, level}.
- 🔧 `update_conf` — Persist a rule's confirmation progress (breach timer / candle bucket).
...
- 🔧 `api_level_slots_add_underlying` — …
- 🔧 `api_level_slots_arm` — …
- 🔧 `api_level_slots_chart` — Closed candles at the slot's TF (or ?tf=) + the slot itself for overlays. Read-only.
- 🔧 `api_level_slots_contracts` — Premium-slot strike picker: ATM±n of the nearest expiry (live spot, scrip master).
- 🔧 `api_level_slots_del_underlying` — …
- 🔧 `api_level_slots_disarm` — …
- 🔧 `api_level_slots_list` — …
- 🔧 `api_level_slots_preview` — What-if for the slot as CURRENTLY typed (unsaved form values merged over the saved slot):
- 🔧 `api_level_slots_save` — …
- 🔧 `api_level_slots_search` — F&O underlyings (scrip master OPTIDX+OPTSTK) — never a hardcoded list.
- 🔧 `api_level_slots_watchlist_delete` — …
- 🔧 `api_level_slots_watchlist_save` — Upsert {name, symbols[]}. Symbols are validated against the F&O universe (or BTC).
- 🔧 `api_level_slots_watchlists` — …
- 🔧 `api_log` — …
- 🔧 `api_lot_sizes` — …
